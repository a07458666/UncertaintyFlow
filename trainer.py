...
class UncertaintyTrainer:
    def __init__(self, config, device) -> None:
        ...
        self.loadDataset(config)
        self.creatModel(config)
        self.defOptimizer(config)
        return

    def loadDataset(self, config) -> None:
        ...
        if (config["add_uniform"]):
            ...
            self.uniform_count = int(config["batch"] * config["uniform_rate"])
        ...
        if config["position_encode"]:
            ...
            self.cond_size += (config["position_encode_m"] * 2)
    ...
    def creatModel(self, config) -> None:
        if config["linear_encode"]:
            self.prior = cnf(config["inputDim"], config["flow_modules"], self.cond_size, 1, config["linear_encode_m"])
        else:
            self.prior = cnf(config["inputDim"], config["flow_modules"], self.cond_size, 1)
        return

    def defOptimizer(self, config) -> None:
        self.optimizer = optim.Adam(self.prior.parameters(), lr=config["lr"])
        return

    def fit(self, config) -> list:
        self.prior.train()
        loss_list = []
        ...
        with tqdm(range(config["epochs"])) as pbar:
            for epoch in pbar:
                for i, x in enumerate(self.train_loader):
                    ...
                    if (config["add_uniform"]):
                        input_y, condition_X = addUniform(input_y, condition_X, self.uniform_count, self.X_mean, self.y_mean, self.X_var, self.y_var, config)
                ...
                self.save(f'result/{config["output_folder"]}/flow_{str(epoch).zfill(2)}.pt')
        return loss_list
    ...
    def loadEvalDataset(self, config):
    ...
    def sample(self, config) -> None:
        ...
        for i, x in tqdm(enumerate(self.evalset)):
            input_x = torch.normal(mean = 0.0, std = 1.0, size=(config["sample_count"] ,1)).unsqueeze(1).to(self.device)
        ...
        savePath = f'result/{config["output_folder"]}/var.png'
